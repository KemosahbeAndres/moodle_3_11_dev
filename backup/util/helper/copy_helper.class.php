...
final class copy_helper {
     ...
     * Sets up relevant controller and adhoc task.
     *
    ...
    public static function get_copies(int $userid, ?int $courseid = null): array {
        ...
        // Our SQL only gets controller that have not finished successfully.
        ...
        foreach ($copyrecords as $copyrecord) {
            try {
                ...
                // The mapping is guaranteed to exist for restore controller, but not
                // backup controller.
                //
            ...
            // The backup cleanup task will take care of updating/deleting invalid controller.
            if ($isbackup) {
     ...
     * @return array An array of mappings between backup ids and restore controller
     */
    ...
    /**
     * Detects and deletes/fails controller associated with a course copy that are
     * in an invalid state.
     *
     * @param array $backuprecords An array of records from {backup_controllers}
